...
interface ThumbnailCategoryItem {
  _id: string;
  name: string;
  slug: string;
  isActive: boolean;
...
export default function AdminThumbnailCategoriesPage() {
  const [items, setItems] = useState<ThumbnailCategoryItem[]>([]);
  const [loading, setLoading] = useState(true);
  const [showForm, setShowForm] = useState(false);
  const [editing, setEditing] = useState<ThumbnailCategoryItem | null>(null);
  ...
  const fetchCategories = useCallback(async () => {
    try {
      ...
      console.error('Failed to fetch thumbnail categories:', err);
    } finally {
      setLoading(false);
    }
  }, []);
  ...
  const openEditForm = (item: ThumbnailCategoryItem) => {
    setEditing(item);
    setName(item.name);
    setShowForm(true);
    setError('');
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      if (editing) {
        const res = await fetch(`/api/admin/thumbnail-categories/${editing._id}`, {
          method: 'PATCH',
  ...
  const handleDelete = async (id: string) => {
    if (!confirm('Are you sure you want to delete this thumbnail category?')) return;

    try {
      const res = await fetch(`/api/admin/thumbnail-categories/${id}`, { method: 'DELETE' });
      const data = await res.json();
      if (data.success) {
  ...
  const handleToggleActive = async (item: ThumbnailCategoryItem) => {
    try {
      const res = await fetch(`/api/admin/thumbnail-categories/${item._id}`, {
        method: 'PATCH',
        headers: { 'Content-Type': 'application/json' },
    ...
    <AdminShell>
      <div className="space-y-6">
        <div className="flex items-center justify-between">
          <div>
            <h1 className="text-2xl font-bold text-white">Thumbnail Categories</h1>
            <p className="text-slate-400 mt-1">Manage categories for thumbnail portfolio items</p>
          </div>
          <button
            ...
            <div className="bg-slate-900 border border-slate-800 rounded-2xl w-full max-w-md">
              <div className="flex items-center justify-between p-6 border-b border-slate-800">
                <h2 className="text-lg font-semibold text-white">
                  {editing ? 'Edit Thumbnail Category' : 'Add Thumbnail Category'}
                </h2>
              ...
              <form onSubmit={handleSubmit} className="p-6 space-y-4">
                ...
                <div>
                  ...
                  <input
                    ...
                    placeholder="e.g., YouTube, Podcast, Gaming"
                  />
                </div>
        ...
        {loading ? (
          ...
          <div className="text-center py-16 bg-slate-900 border border-slate-800 rounded-2xl">
            ...
            <p className="text-slate-400">No thumbnail categories yet</p>
            <button
              onClick={() => { resetForm(); setShowForm(true); }}
